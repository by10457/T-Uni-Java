...
`server-services/server-im` 是可选模块，默认关闭。它只沉淀 OpenIM 的通用模板能力，包括配置下发、用户同步、token 获取、webhook 验签和系统通知直发，不复制 `wxy-server` 的社交业务逻辑。
...
OpenIM 已按可选模块方式沉淀为最小模板能力，但社交私信、社交通知回流、内容审核和隐私策略仍不纳入主模板。后续规划见 [../TODO.md](../TODO.md)。
