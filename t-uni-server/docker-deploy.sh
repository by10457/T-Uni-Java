...
echo "========================================="
echo "  T-Uni Server Docker 部署脚本"
echo "========================================="

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
PROJECT_ROOT=$(cd "$SCRIPT_DIR/.." && pwd)

IMAGE_NAME="${IMAGE_NAME:-t-uni-server}"
CONTAINER_NAME="${CONTAINER_NAME:-t-uni-server}"
MODULE_PATH="t-uni-server/server-api"
JAR_FILE="$PROJECT_ROOT/$MODULE_PATH/target/t-uni-server.jar"
DOCKERFILE="$SCRIPT_DIR/Dockerfile"
PORT="${PORT:-10457}"
SPRING_PROFILE="${SPRING_PROFILE:-prod}"
ENV_FILE="${ENV_FILE:-$PROJECT_ROOT/.env}"
HOST_LOG_DIR="${HOST_LOG_DIR:-$PROJECT_ROOT/logs/t-uni-server}"
...
echo "[1/6] 检查 jar 包..."
if [ ! -f "$JAR_FILE" ]; then
  echo "错误: 未找到 jar 包，请先执行 ./t-uni-server/build.sh" >&2
...
echo "========================================="
echo "容器名称: $CONTAINER_NAME"
echo "访问地址: http://localhost:$PORT/api/health"
echo "运行环境: $SPRING_PROFILE"
