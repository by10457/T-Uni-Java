...
- **JWT** （认证授权，Admin 使用 Spring Security，Server 使用自定义拦截器）
...
│   ├── common-core/           # 核心工具类、JWT、异常处理
│   ├── common-config/         # 中间件配置
│   └── common-web/           # Web通用配置
├── t-uni-admin/              # 后台管理系统
│   ├── admin-domain/         # 领域模型
│   ├── admin-common/         # Admin通用工具
│   ├── admin-services/       # 业务服务
│   │   ├── system/           # 系统管理服务
...
│   └── admin-api/            # API入口
└── t-uni-server/             # 小程序后端
    ├── server-common/        # Server专属通用模块（含小程序JWT扩展）
    ├── server-domain/        # 领域模型
...
**职责**：Admin通用工具（依赖common-core和common-config）
...
**认证方式**：JWT + AuthInterceptor（**已实现双Token机制**，详见 `server-common` 模块）
**登录方案**：详见 `WX_LOGIN_PLAN.md`
...
BaseException(ResultCodeEnum.USER_NOT_FOUND);
}
```
...
1. **模块重命名**
    - `admin-core-common` → `admin-common`（简化命名）

2. **删除无用模块**
    - 删除 `server-payment` 空模块
    - 移除 `server-core-common` 无效依赖声明

3. **安全改进**
    - 修复 `JwtKeyHolder` 在 JDK 17+ 的反射兼容性问题（使用 AtomicReference）
    - 从 `t-uni-server` 移除不必要的 `spring-boot-starter-security` 依赖

### 2026-01-08 更新

1. **消除重复代码**
    - 删除重复的 `JwtTokenUtil.java`，统一使用 `common-core` 版本

2. **创建扩展模块**
    - 在 `server-common` 创建 `ServerJwtTokenUtil.java`，支持：
        - 小程序用户 token 生成
...
| 工具类                  | 位置            | 用途                        |
|----------------------|---------------|---------------------------|
| `JwtTokenUtil`       | common-core   | 通用 JWT 工具，Admin/Server 共享 |
| `ServerJwtTokenUtil` | server-common | 小程序专用扩展（双Token、微信用户信息）    |

### 开发注意事项

- **JWT 使用**：通用功能用 `t.uni.common.core.utils.JwtTokenUtil`
- **小程序扩展**：使用 `t.uni.server.common.utils.ServerJwtTokenUtil`
